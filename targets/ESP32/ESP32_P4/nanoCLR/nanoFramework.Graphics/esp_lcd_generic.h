...
//
// LCD panel initialization command item.
//
typedef struct {
...
} generic_lcd_init_cmd_t;

//
// LCD panel vendor configuration.
//
typedef struct {
    const generic_lcd_init_cmd_t *init_cmds;         /*!< Pointer to initialization commands array. Set to NULL if using default commands.
...
} generic_vendor_config_t;

//
// Create Generic LCD panel
//
// io - LCD panel IO handle(in)
// panel_dev_config - General panel device configuration(in)
// ret_panel - Returned LCD panel handle(out)
// 
// return
...
#define GENERIC_DEFAULT_PANEL_BUS_DSI_2CH_CONFIG()                \
    {                                                    \
        .bus_id = 0,                                     \
        .num_data_lanes = 2,                             \
...
//
// MIPI-DBI panel IO configuration structure
//
#define GENERIC_DEFAULT_PANEL_IO_DBI_CONFIG()  \
    {                                 \
        .virtual_channel = 0,         \
        .lcd_cmd_bits = 8,            \
        .lcd_param_bits = 8,          \
 ...
 *
 */
#define GENERIC_800_1280_PANEL_60HZ_DPI_CONFIG(px_format) \
    {                                                    \
